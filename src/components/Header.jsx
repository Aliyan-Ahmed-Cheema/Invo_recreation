...
// Helper component for portfolio tags
    ...
    return (
        <header className="header-wrapper" onMouseLeave={handleMenuLeave}>
            <div className="header-main">
                {/* Simplified layout: Three direct children of a flex container */}
            ...
            {/* --- MEGA MENU OVERLAY (Now includes all rendering logic) --- */}
                    ...
                    {currentMenuData.layout === 'tabs' && (
                        <div className={`mega-menu-content-area ${!currentMenuData.sideCard && !currentMenuData.sidebarCta ? 'full-width' : ''}`}>
                            ...
                            <div className="main-content">
                                {activeSubMenu && menuData[activeMenu].subMenus[activeSubMenu].content.type === 'links' &&
                                    <div className="content-grid-links">{menuData[activeMenu].subMenus[activeSubMenu].content.items.map(item => (<a href="#" key={item.title}><h4>{item.title}</h4><p>{item.text}</p></a>))}</div>}
                                {activeSubMenu && menuData[activeMenu].subMenus[activeSubMenu].content.type === 'grid' &&
                                    <div className="content-grid-tags">{menuData[activeMenu].subMenus[activeSubMenu].content.items.map(item => (<a href="#" key={item}>{item}</a>))}</div>}
                                {activeSubMenu && menuData[activeMenu].subMenus[activeSubMenu].content.type === 'portfolio' &&
                                    <div className="content-grid-portfolio">{menuData[activeMenu].subMenus[activeSubMenu].content.items.map(item => (<a href="#" key={item.title} className="portfolio-item"><div className="portfolio-item-header"><h4>{item.title}</h4><PortfolioTag tag={item.tag} /></div><p>{item.text}</p></a>))}</div>}
                            </div>
